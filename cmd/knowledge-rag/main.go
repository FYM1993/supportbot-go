...
func main() {
	...
	r.POST("/api/rag", func(c *gin.Context) {
		...
		go processRAG(req, llmClient, imDemoURL, zapLogger)
	...
	zapLogger.Info("knowledge-rag 服务启动成功", zap.Int("port", cfg.Server.Port))
...
func processRAG(req RAGRequest, llmClient *client.DashScopeClient, 
	...
	产品使用手册摘要：
	1. 开机：长按电源键 3 秒
	2. 充电：使用原装充电器，约需 2 小时
	3. 保养：避免接触水，定期清洁
	`

	systemPrompt := "你是知识库助手，根据检索到的知识回答用户问题。"
	prompt := fmt.Sprintf("知识库内容：%s\n\n用户问题：%s", knowledgeContext, req.Question)
	
	response, err := llmClient.SimpleChat(systemPrompt, prompt)
	if err != nil {
		logger.Error("LLM 调用失败", zap.Error(err))
		response = "抱歉，暂时无法查询到相关信息。"
	}
